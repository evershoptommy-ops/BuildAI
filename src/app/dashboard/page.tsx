...
export default async function DashboardPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-9 py-5" style={{ borderBottom: '1px solid #1e1e30' }}>
        <div>
        ...
        <div className="flex items-center gap-3">
          {streak > 0 && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 14px', borderRadius: 999, fontSize: 13, fontWeight: 500, background: 'rgba(245,158,11,.12)', color: '#fbbf24', border: '1px solid rgba(245,158,11,.25)' }}>
              <Flame size={14} /> {streak}-daagse streak
            </div>
          )}
          <Link href={`/modules/${activeModule.id}/${nextLesson.id}`}>
            <button style={{ background: '#7c3aed', border: 'none', color: '#fff', borderRadius: 10, padding: '10px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div className="p-9">
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div style={{ background: '#111118', border: '1px solid #1e1e30', borderRadius: 14, padding: '22px 24px' }}>
